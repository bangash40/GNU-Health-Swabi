...
class Statement(Workflow, ModelSQL, ModelView):
    ...
    def _get_grouped_line(self):
        ...
        class Line(namedtuple('Line', keys + ['lines'])):
            ...
            @property
            def serial_no(self):
                return sum((l.serial_no for l in self.lines)) 

            @property
            def line_type(self):
                done = set()
                for line in self.lines:
                    if line.line_type and line.line_type not in done:
                        done.add(line.line_type)
            ...
            def visit_no(self):
                ...
                for line in self.lines:
                    if line.sale and line.sale.id not in done:
                        done.add(line.sale.id)
                        yield line.sale.get_token_number() if line.sale else ''
            ...
            def receipt_no(self):
                ...
                for line in self.lines:
                    if line.sale and line.sale.id not in done:
                        ...
                        yield line.sale.number if line.sale else ''
            ...
            def sale_discount(self):
                ...
                for line in self.lines:
                    if line.sale and line.sale.id not in done:
                        ...
                        yield line.sale.get_total_discount() if line.sale else 0                               
        return Line
    ...
    def create_move(cls, statements):
        ...
        moves = []
        for statement in statements:
            # try to remove those lines from statement.lines of each statement whose type is credit_sale
            statement_lines_non_credit = []
            for line in statement.lines:
                if line.line_type == 'credit_sale':
                    continue
                else:
                    statement_lines_non_credit.append(line)
            #for key, lines in groupby(
            #        statement.lines, key=statement._group_key):
            for key, lines in groupby(
                    statement_lines_non_credit, key=statement._group_key):
...
class StatementReport(CompanyReport):
    ...
    def get_context(cls, records, header, data):
        cash_sale = 0
        report_context = super(StatementReport, cls).get_context(records, header, data)
        balance = 0
        for r in records:
                balance = 0
                counter = 1
                ...
                for line in r.lines:
                    ...
                    line.serial_no = counter
                    counter +=1
                    if line.sale:
                        sale_discount_total = sale_discount_total + line.sale.get_total_discount()
                        if line.sale.welfare_discount_value:
                            sale_discount_total = sale_discount_total + line.sale.welfare_discount_value
                            
                    #line.line_type = line.line_type

                    if line.line_type == 'cash_sale':
                        cash_sale += line.amount
                    if line.line_type == 'credit_sale':
                        credit_sale += line.amount
                    if line.line_type == 'sale_refund':
                        sale_refund += line.amount

                    if line.line_type == 'ipd_admission':
                        ipd_admission += line.amount
                        ipd_total += line.amount
                    if line.line_type == 'ipd_advance':
                        ipd_advance += line.amount
                        ipd_total += line.amount
                    if line.line_type == 'ipd_discharge':
                        ipd_discharge += line.amount
                        ipd_total += line.amount
                    
                report_context['cash_sale'] = cash_sale
                report_context['credit_sale'] = credit_sale
                report_context['sale_refund'] = sale_refund
                report_context['net_cash_sale'] = cash_sale + sale_refund
                report_context['sale_discount_total'] = sale_discount_total

                report_context['ipd_admission'] = ipd_admission
                report_context['ipd_advance'] = ipd_advance
                report_context['ipd_discharge'] = ipd_discharge
                report_context['ipd_total'] = ipd_total
